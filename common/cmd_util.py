...
def make_vec_env(env_id, env_type, num_env, seed, wrapper_kwargs=None, start_index=0, reward_scale=1.0, gamestate=None):
    ...
    if num_env > 1:
        return SubprocVecEnv([make_thunk(i + start_index) for i in range(num_env)])     #make_thunk返回一组env，只有subrank不同
    else:
...
def common_arg_parser():
    ...
    """
    parser = arg_parser()
    parser.add_argument('--env', help='environment ID', type=str, default='MontezumaRevengeNoFrameskip-v0')
